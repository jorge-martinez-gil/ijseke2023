...
# Define categories to use as features
...
def protected_division(x, y):
    """
    Division function that handles division by zero.
    """
    return np.divide(x, y, out=np.copy(x), where=y!=0)

def my_pearson(x, y):
    """
    Calculates the negative Pearson correlation coefficient between two arrays.
    """
    a = x.flatten()
    b = y.flatten()
...
def my_spearman(x, y):
    """
    Calculates the negative Spearman correlation coefficient between two arrays.
    """
    a = x.flatten()
    b = y.flatten()
...
hyperparameters = {
    'n_columns': [50, 100, 150, 200, 250, 300],
    'n_rows': [1, 2, 3, 4]
}
...
# Predict on test data and print Pearson correlation coefficient
